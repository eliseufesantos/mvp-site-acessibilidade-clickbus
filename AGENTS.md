...
- suíte do agente de acessibilidade: 40 testes aprovados, incluindo contrato do adaptador Gemini com transporte falso, origem, limite de corpo, quota local, handler/URL Rybená, contrato do adaptador com runtime falso, adaptador de desenvolvimento, contrato 2.1 e a garantia de que o executor não toca nos métodos visuais da Rybená;
...
- painel de acessibilidade reconstruído no padrão **lançador + superfície** em 19/09: acionador circular no header, grade 2×2 de cartões e chat. O `tablist` de três abas não existe mais. Em 20/09 a raiz passou a ter duas vistas com uma chave entre elas, abrindo no assistente;
...
- contrato do agente na versão **2.2**, com as seis ações de voz e as duas de conteúdo. O servidor responde `400` a um pedido `2.0` e o executor rejeita um plano `2.0` antes de qualquer efeito;
...
- **Um único chat atende ajuste e dicionário.** "aumente o texto" e "o que é viação" entram pelo mesmo campo. Quem decide o destino é o **plano**, com as ações `explain_term` e `simplify_content` — nunca heurística de string no cliente. Não recrie uma superfície de Conteúdo separada.
- O executor tenta o **glossário determinístico e a simplificação revisada antes de qualquer rede**. Um termo conhecido responde na hora, sem gastar cota e sem depender da IA, cuja qualidade semântica segue `PARCIAL`. Quando a explicação vem da IA, a interface pede que se confira antes de usar. A procedência do que é determinístico **não** é anunciada: decisão de 20/09/2026 do responsável — quem pergunta quer a informação, não o funcionamento interno do protótipo, e as especificidades ficam para a apresentação.
...
- o painel é lançador + superfície: uma superfície ativa por vez, `root` por padrão, e abrir sempre começa na raiz. **A raiz tem duas vistas e uma chave entre elas: `Assistente` (o chat) e `Recursos` (a grade 2×2 — Libras, Voz, Ajustes visuais e Sobre). A vista inicial é o assistente**, decisão de 20/09/2026: o protótipo é IA-first e manter a grade sempre na tela custava cerca de 280 px de altura, que empurravam o campo e os botões para fora do alcance. Cartão que navega é `<button>`, e a chave são dois `<button>` com `aria-pressed` — **nunca** `role="tab"`; não recrie o `tablist`.
...
- o chat é a vista inicial da raiz e continua visível na base de `settings` e `about`. Na vista `Recursos` ele cede o lugar à grade — é a única troca deliberada, feita pela chave, e o risco de a IA estar fora do ar foi assumido pelo responsável por ser protótipo de conceito. Quando o serviço de IA falha por indisponibilidade, cota ou sobrecarga, o chat oferece um atalho visível para a vista `Recursos`, para que o caminho que funciona sem rede não fique escondido. Os chips de sugestão apenas preenchem o campo — nunca enviam.
...
- glossário determinístico, registro estático de conteúdo público e simplificações locais revisadas para os alvos iniciais, alcançáveis pelo chat por `explain_term` e `simplify_content`;
